...
class DamageReportController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $eligibleReservations = Reservation::query()
            ...
            ->whereHas('inspections', function ($query) {
                $query->where('inspection_type', InspectionType::PICKUP)
                    ->where('status', InspectionStatus::COMPLETED);
            })
            ->whereHas('inspections', function ($query) {
                $query->where('inspection_type', InspectionType::RETURN)
                    ->where('status', InspectionStatus::COMPLETED);
            })
    ...
    public function store(Request $request, Reservation $reservation)
    {
        ...
        return redirect()->route('admin.damage-reports.show', $damageReport)
