...
import { index as damageReportsIndex, show as damageReportsShow } from '@/routes/admin/damage-reports';
import { create as damageReportsCreate } from '@/routes/admin/damageReports';

const props = defineProps<{
  damageReports: any
  eligibleReservations: any[]
...
<template>
  ...
  <AdminLayout>
    <main class="flex-1 p-8 space-y-6">
      ...
      <div v-if="eligibleReservations.length" class="rounded-md border bg-white p-4 space-y-3">
        <h2 class="text-sm font-semibold text-gray-800">Ready for Damage Report Creation</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div v-for="reservation in eligibleReservations" :key="reservation.id" class="rounded border p-3 flex items-center justify-between">
            <div class="text-sm">
              <div class="font-medium">{{ reservation.reservation_number }}</div>
              <div class="text-gray-600">{{ reservation.user?.name }} - {{ reservation.car?.year }} {{ reservation.car?.make }} {{ reservation.car?.model }}</div>
            </div>
            <Link :href="damageReportsCreate(reservation.id).url">
        ...
        <table v-else class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="report in damageReports.data" :key="report.id" class="border-b hover:bg-gray-50">
              ...
              <td class="px-4 py-3">{{ report.estimated_cost ? `UGX ${Number(report.estimated_cost).toLocaleString('en-US', { maximumFractionDigits: 0 })}` : '—' }}</td>
              ...
              <td class="px-4 py-3">
                <Link :href="damageReportsShow(report.id).url">
