...
import { ref } from 'vue';
import { index as damageReportsIndex, update as damageReportsUpdate } from '@/routes/admin/damage-reports';
import { resolve as damageReportResolve, reject as damageReportReject } from '@/routes/admin/damageReports';
import { show as disputesShow } from '@/routes/admin/disputes';
...
function submitEdit() {
  editForm.put(damageReportsUpdate(props.damageReport.id).url, {
    onSuccess: () => (showEditForm.value = false),
  });
}

function submitResolve() {
  resolveForm.post(damageReportResolve(props.damageReport.id).url, {
    onSuccess: () => (showResolveForm.value = false),
  });
}

function submitReject() {
  rejectForm.post(damageReportReject(props.damageReport.id).url, {
  ...
  <AdminLayout>
    <main class="flex-1 p-8 space-y-6">
      <div class="flex items-center justify-between gap-4">
        ...
        <Link :href="damageReportsIndex().url">
      ...
      <div v-if="activeTab === 'details'" class="space-y-6">
          ...
          <div class="rounded-md border bg-white p-4">
            ...
            <div class="space-y-3 text-sm">
              ...
              <div>
                ...
                <span class="block font-medium"
                  >UGX {{ damageReport.estimated_cost ? Number(damageReport.estimated_cost).toLocaleString('en-US', { maximumFractionDigits: 0 }) : '0' }}</span
                >
              </div>
            </div>
          </div>
        ...
        <div v-if="!showEditForm && !showResolveForm && !showRejectForm" class="flex gap-2 flex-wrap">
          ...
          <Link v-if="damageReport.disputes && damageReport.disputes.length > 0" :href="disputesShow(damageReport.disputes[0].id).url">
      ...
      <div v-if="activeTab === 'disputes' && damageReport.disputes && damageReport.disputes.length > 0">
        <div class="space-y-4">
          <Link
            ...
            :key="dispute.id"
            :href="disputesShow(dispute.id).url"
